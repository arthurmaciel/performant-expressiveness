(load-shared-object "./libforchez.so")

(define perf-counter
  (foreign-procedure "clock_now" () double))

(define print
  (foreign-procedure "internal_printf" (string string double) void))
...
(define (print-perf test-name bench-result)
  (print "chez" test-name bench-result))
...
(define (parse-int t)
  (for i in (iota t)
       (let* ((n (random (- (expt 2 32) 1)))
              (s (number->string n 16))
              (m (string->number s 16)))
         (unless (= n m) (error 'parse-int "Incorrect parsing of integers!" 'parse-int))
         n)))
